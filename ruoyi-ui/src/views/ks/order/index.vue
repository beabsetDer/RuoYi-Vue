    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="機司名稱" prop="oilDriver">
          ...
          -->
          <el-select clearable v-model="form.oilDriver" placeholder="請選擇機司名稱">
            <el-option
              v-for="item in userList"
              :key="item.employeeId"
        ...
        <el-form-item label="油加車牌" prop="oilLicensePlateNumber">
          ...
          <el-select clearable v-model="form.oilLicensePlateNumber" placeholder="請選擇車牌">
            <el-option
              v-for="car in carList"
              :key="car.licensePlateNumber"
              :value="car.licensePlateNumber"
...
export default {
  ...
  methods: {
    ...
    countOil() {
      ...
      let sumPrice = this.form.oilPrice * this.form.oilLiters;
    ...
    getList() {
      ...
      console.info("YYYY", this.dateRange);
    },
    // 取消按钮
    cancel() {
      this.open = false;
    ...
    handleAdd() {
      ...
      // 台北時間+8小時
      this.form.oidDate = new Date(+new Date() + 8 * 3600 * 1000)
        .toISOString()
        .slice(0, 10);
