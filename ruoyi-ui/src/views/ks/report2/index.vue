    ...
    <div id="printCons">
      <el-dialog title="個人車趟表單詳細資訊" :visible.sync="dialog.allData" width="90%">
        ...
        <div align="right">
          ...
          <el-button
            type="primary"
            v-hasPermi="['ks:employee:exportSalary']"
...
<script>
import { listReport, emplistReport, exportSalary } from "@/api/ks/report";
...
export default {
  ...
  methods: {
    ...
    eeeeeeee(employeeId) {
      ...
      console.info(this.queryParams);
      console.info(id);
      //  exportSalary(this.addDateRange(this.queryParams, this.dateRange), id);

      this.download(
        "ks/employee/exportSalary",
        {
          ...this.addDateRange(this.queryParams, this.dateRange),
          id
        },
        `downbound_${new Date()}.xlsx`
      );
    },
