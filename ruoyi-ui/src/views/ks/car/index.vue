<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="carList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="購車日期" align="center" prop="carPurchaseDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.carPurchaseDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="貸款狀態" align="center" prop="carLoan" />
      <el-table-column label="保養日期" align="center" prop="carMaintenanceDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.carMaintenanceDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="下次保養日期" align="center" prop="nextMaintenanceDate" width="180">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除車輛表單编号为"' + ids + '"的数据项？').then(function() {
        return delCar(ids);
      }).then(() => {
        this.getList();
